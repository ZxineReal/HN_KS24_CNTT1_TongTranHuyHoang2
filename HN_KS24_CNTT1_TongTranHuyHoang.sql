...
select room_type, count(room_type) as total_room from rooms
group by room_type;
...
select g.guest_id, g.guest_name, g.phone, count(b.guest_id) as total_bookings from guests g
...
select r.room_id, r.room_type, r.price_per_day, sum((b.check_out - b.check_in) * r.price_per_day) as total from rooms r
join bookings b on b.room_id = r.room_id
group by b.room_id;

-- Hiển thị tổng doanh thu của từng loại phòng

select r.room_type, sum((b.check_out - b.check_in) * r.price_per_day) as total from rooms r
join bookings b on b.room_id = r.room_id
group by r.room_type;

-- Tìm những khách đã đặt phòng từ 2 lần trở lên

select g.guest_id, g.guest_name, g.phone, count(g.guest_id) as total_booking from guests g
join bookings b on b.guest_id = g.guest_id
group by g.guest_id
